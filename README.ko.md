...
측정된 설치 파일은 **3.5 MB**, 설치 후 용량은 **18 MB**입니다. 무인 설치에는 **약 2.3초**, 실행 중인 복사본 위에 재설치할 때는 **약 2.8초**가 걸립니다. 상태 표시줄 콜드 스타트는 **약 37ms**입니다. WebView2가 필요하며 Windows 11과 최신 Windows 10에는 기본으로 포함되어 있습니다.

### 직접 빌드하기

LazySwitch는 Tauri v2 앱입니다. Rust가 앱과 공유 코어, 네이티브 CLI를 담당하고, `src/renderer/`의 HTML/CSS/JS UI는 WebView2를 통해 표시됩니다. 렌더러가 사용하는 API는 `src/renderer/tauri-shim.js`가 다시 구성합니다. Rust 소스는 `src-tauri/`에 있으며, 공유 코어는 `src-tauri/src/core/`, CLI는 `src-tauri/src/bin/lazyswitch-cli.rs`에 있습니다.
...
- Windows 10 / 11
- WebView2 — Windows 11과 최신 Windows 10에 기본으로 포함됨
...
- Rust 툴체인과 Tauri CLI — 소스에서 빌드할 때만
...
트레이 앱이 본체지만, 같은 수치를 터미널에서도 볼 수 있습니다. 네이티브 `lazyswitch-cli.exe`는 앱과 함께 `%LOCALAPPDATA%\LazySwitch\lazyswitch-cli.exe`에 설치됩니다. 해당 폴더에서 `lazyswitch-cli.exe`로 실행하거나 전체 경로를 사용하세요.

```powershell
lazyswitch-cli.exe status                    # 계정 사용량 표를 한 번 출력
lazyswitch-cli.exe watch --interval 30       # 표를 계속 갱신하며 표시
lazyswitch-cli.exe statusline all            # 계정별 한 줄 요약
lazyswitch-cli.exe statusline codex          # 특정 프로바이더
lazyswitch-cli.exe statusline claude         # 특정 프로바이더
lazyswitch-cli.exe install-hooks             # Claude statusLine + Codex [tui] 키
```

명령어는 `status`, `watch [--interval N]`, `statusline [codex|claude|all]`, `install-hooks`입니다. CLI는 별도 런타임 의존성이 없는 네이티브 바이너리입니다. `install-hooks`는 Claude Code의 `statusLine`이 이 바이너리를 가리키도록 설정하고, Codex의 `[tui]` 키를 기록하며, 두 설정 파일을 먼저 백업합니다.
