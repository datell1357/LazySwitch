...
fn lookup(lang: Lang, key: &str) -> Option<&'static str> {
    dict(lang).iter().find(|(k, _)| *k == key).map(|(_, v)| *v)
}

/// Translate `key` in `lang`, substituting {var} placeholders from `vars`.
/// Each placeholder is replaced at most once, matching the original
/// String.prototype.replace (non-global) semantics.
pub fn t(lang: Lang, key: &str, vars: &[(&str, &str)]) -> String {
    let raw = lookup(lang, key)
    ...
    for (k, v) in vars {
        ...
        if let Some(pos) = s.find(&needle) {
            s.replace_range(pos..pos + needle.len(), v);
        }
    }
    s
}
...
static EN: &[(&str, &str)] = &[
    ...
    ("tray.tutorial", "Show the tutorial…"),
    ...
    ("tray.usageWidget", "Show usage widget"),
    ...
    (
        "notif.desktopFailedBody",
        "Could not locate the Codex Desktop executable.",
    ),
    ("notif.cliRestartedTitle", "CLI sessions restarted"),
    (
        "notif.cliRestartedBody",
        "{provider}: {count} reopened in a new terminal; {closed} old terminals closed.",
    ),
    (
        "notif.cliRestartedManualBody",
        "{provider}: {count} reopened in a new terminal; {closed} old terminals closed; {manual} need manual resume. Resume command copied.",
    ),
    ("notif.cliCommandCopiedTitle", "Resume command copied"),
    (
        "notif.cliCommandCopiedBody",
        "Paste {command} in any running {provider} terminal.",
    ),
...
static KO: &[(&str, &str)] = &[
    ...
    (
        "notif.desktopFailedBody",
        "Codex 데스크탑 실행 파일을 찾을 수 없습니다.",
    ),
    ("notif.cliRestartedTitle", "CLI 세션 재시작됨"),
    (
        "notif.cliRestartedBody",
        "{provider}: 새 터미널에서 재개 {count}개, 기존 터미널 종료 {closed}개.",
    ),
    (
        "notif.cliRestartedManualBody",
        "{provider}: 새 터미널에서 재개 {count}개, 기존 터미널 종료 {closed}개, 수동 재개 필요 {manual}개. 명령을 클립보드에 복사했습니다.",
    ),
    ("notif.cliCommandCopiedTitle", "재개 명령 복사됨"),
    (
        "notif.cliCommandCopiedBody",
        "{provider} 터미널에 {command} 명령을 붙여 넣으세요.",
    ),
...
static JA: &[(&str, &str)] = &[
    ...
    (
        "notif.desktopFailedBody",
        "Codex デスクトップの実行ファイルが見つかりません。",
    ),
    ("notif.cliRestartedTitle", "CLI セッションを再起動しました"),
    (
        "notif.cliRestartedBody",
        "{provider}: 新しいターミナルで {count} 個を再開し、既存ターミナル {closed} 個を閉じました。",
    ),
    (
        "notif.cliRestartedManualBody",
        "{provider}: 新しいターミナルで {count} 個を再開し、既存ターミナル {closed} 個を閉じました。{manual} 個は手動再開が必要です。再開コマンドをコピーしました。",
    ),
    ("notif.cliCommandCopiedTitle", "再開コマンドをコピーしました"),
    (
        "notif.cliCommandCopiedBody",
        "{provider} のターミナルに {command} を貼り付けてください。",
    ),
...
static ZH: &[(&str, &str)] = &[
    ...
    (
        "notif.cliRestartedBody",
        "{provider}：已在新终端恢复 {count} 个，已关闭原终端 {closed} 个。",
    ),
    (
        "notif.cliRestartedManualBody",
        "{provider}：已在新终端恢复 {count} 个，已关闭原终端 {closed} 个；{manual} 个需要手动恢复。恢复命令已复制。",
    ),
...
mod tests {
    ...
    #[test]
    fn resolves_supported_prefs_case_insensitively() {
        assert_eq!(resolve_lang("KO"), Lang::Ko);
        assert_eq!(resolve_lang("ja-JP"), Lang::Ja);
        assert_eq!(resolve_lang("zh-CN"), Lang::Zh);
        assert_eq!(resolve_lang("fr"), Lang::En);
    }

    #[test]
    fn all_languages_have_matching_key_sets() {
        let en_keys: Vec<&str> = EN.iter().map(|(k, _)| *k).collect();
        for (name, d) in [("ko", KO), ("ja", JA), ("zh", ZH)] {
            let keys: Vec<&str> = d.iter().map(|(k, _)| *k).collect();
            assert_eq!(keys, en_keys, "{name} key set diverges from en");
        }
    }

    #[test]
    fn substitutes_each_placeholder_once() {
        let s = t(Lang::En, "tray.left", &[("label", "Codex"), ("pct", "42"), ("time", "3pm")]);
        assert_eq!(s, "Codex: 42% left (resets 3pm)");
    }

    #[test]
    fn falls_back_to_english_for_missing_key_in_lang() {
        // popup.title exists in all dicts; sanity check unknown key returns the key itself.
        assert_eq!(t(Lang::Ko, "no.such.key", &[]), "no.such.key");
    }
}
