/// PowerShell script that enumerates processes matching `__PROCESS_NAME__`
/// and reads each one's current working directory straight out of its PEB
/// (Process Environment Block) via P/Invoke — Windows exposes no public API
/// for a process's cwd, so this is the same technique Sysinternals-style
/// tools use. Ported verbatim from the original TS string constant: it is
/// already a self-contained PowerShell/C# script, so there is nothing
/// Electron-specific to remove — only the call site (execFile) changes.
pub const PEB_CWD_SCRIPT: &str = r#"
